...
def params(data, flag):
    # Setting the x and values before and after the jump
...
def line_fitting(xs_before_jump, ys_before_jump ,xs_after_jump, ys_after_jump):
    # Linear regression before and after the jump -- fitting slopes to get 'true' offset
...
def lenAfter_less(data, jump_idx, xs_before_jump, ys_before_jump ,xs_after_jump, ys_after_jump):
    """
    Length of points after jump are less than 20 points ==> use whatever points remain to do line fitting
    """
...
def jump_frame1(frames, data, jump_idx, xs_before_jump, ys_before_jump ,xs_after_jump, ys_after_jump):
    """
    If a jump is detected at frame 1 then it finds the offset btwn frames 1 and 0, then brings every frame after down then finds actual offset
    """
...
if __name__ == "__main__":
    ...
    # Open fits file
    utr_dark = fits.open('UTR_dark.fits')
    img_stack = utr_dark[0].data
    frames = np.arange(0, img_stack.shape[0], 1)
    
    corrected_array = np.zeros((img_stack.shape[0]))
        ...
        for j in range(img_stack.shape[2]):
            ...
            if len(flag) > 0:
                for jump in flag:
                    ...
                    plt.plot(corr_pix, color='blue')
                    plt.scatter(frames, corr_pix, color='lightblue')
                    
                    plt.plot(singlePixel, color='green')
                    plt.scatter(frames, singlePixel, color='lime')
                    plt.show()
            
                        
    t2 = time.time()
    total = t2 - t1
    print('Corrections took:', total, 'seconds')
